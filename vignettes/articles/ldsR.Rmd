...
ldsR exposes six functions:
1.  `celltype_analysis()` to estimate partitioned heritability for many annotations one at a time
...
5.  `parse_gwas()` a function to read in summary statistics in the many different formats they can be found in the wild.
6.  `partitioned_h2()` to estimate partitioned heritability
...
# Munge
The `munge()` function can be used to mimic some of the filters of the munge_sumstats.py file from ldsc.
```{r}
munge(sumstat1)
```
