...
#' 
#' 
partitioned_heritability <- function(sumstat, ldscore_dir, weights = NULL, n_blocks=200) {
  stopifnot("sumstat has to be a data.frame or tbl" = "data.frame" %in% class(sumstat))
  ...
  res <- ldscore(y = merged$Z^2, x = x, w = merged$L2, N = merged$N, M = covar_M, n_blocks=n_blocks)
