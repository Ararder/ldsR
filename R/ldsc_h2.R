...
#'
#'
ldsc_h2 <- function(sumstat, pop_prev=NULL, sample_prev=0.5, weights=NULL, M=NULL, n_blocks = 200) {
  req_cols <- c("SNP", "Z", "N")
  stopifnot("sumstat has to be a data.frame or tbl" = "data.frame" %in% class(sumstat))
  check_columns(req_cols, sumstat)
  stopifnot("sample_prev must be a scalar double" = rlang::is_scalar_double(sample_prev))
  ...
  if(!is.null(pop_prev)) {
    out_df <- out_df |>
      dplyr::mutate(
        lia_h2 = liability_h2(h2, pop_prev, sample_prev),
        lia_se = liability_h2(h2_se, pop_prev, sample_prev)
      ) |>
      dplyr::relocate(
        lia_h2, lia_se
      )
